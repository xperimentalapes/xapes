...
const { getDefaultTierXma } = require('../buy-in-tiers');
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (!player) {
            const defaultCost = getDefaultTierXma();
            return res.status(200).json(isRoulette
                ? { walletAddress, totalSpins: 0, totalWon: 0, totalWagered: 0, unclaimedRewards: 0, chipsBalance: 0, costPerChip: defaultCost, createdAt: null }
                : { walletAddress, totalSpins: 0, totalWon: 0, totalWagered: 0, unclaimedRewards: 0, spinsRemaining: 0, costPerSpin: defaultCost, createdAt: null }
            );
        }
        ...
        return res.status(200).json(isRoulette
            ? {
                ...
                costPerChip: Number(player.cost_per_chip || defaultCost),
            ...
            : {
                ...
                costPerSpin: player.cost_per_spin || defaultCost,
                createdAt: player.created_at
            }
        );
