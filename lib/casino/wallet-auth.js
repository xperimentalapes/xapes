...
const PLAY_SESSION_ACTION = 'play-session';
const PLAY_SESSION_ACTIONS = new Set(['spin-slots', 'spin-roulette', 'coinflip-flip']);
...
function verifyCasinoAuth(req, action, walletAddress) {
  ...
  if (parts[1] !== action) {
    ...
    if (!(PLAY_SESSION_ACTIONS.has(action) && signedAction === PLAY_SESSION_ACTION)) {
      throw new Error('Signed action mismatch');
    }
  }
