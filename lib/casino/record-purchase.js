...
const { resolveBuyInTiers, isAllowedBuyInCost } = require('./buy-in-tiers');
...
async function recordVerifiedPurchase({
  ...
  const { tiers } = await resolveBuyInTiers();
  if (!isAllowedBuyInCost(cost, tiers)) {
    throw new Error(
      'Invalid buy-in amount. Choose one of: ' +
        tiers.map(function (t) { return t.optionLabel; }).join(', ')
    );
  }

  if (gameType === 'slots') {
    const maxCredits = Math.min(MAX_SPINS_PER_PURCHASE, getMaxCreditsForPurchase(cost));
    if (credits > maxCredits) {
      throw new Error(`Max ${maxCredits} spins at ${cost} XMA each (2M XMA purchase limit)`);
    }
  } else if (gameType === 'roulette') {
    const maxCredits = Math.min(MAX_CHIPS_PER_PURCHASE, getMaxCreditsForPurchase(cost));
    if (credits > maxCredits) {
      throw new Error(`Max ${maxCredits} chips at ${cost} XMA each (2M XMA purchase limit)`);
    }
  } else if (gameType === 'coinflip') {
    const maxCredits = Math.min(MAX_SPINS_PER_PURCHASE, getMaxCreditsForPurchase(cost));
    if (credits > maxCredits) {
      throw new Error(`Max ${maxCredits} flips at ${cost} XMA each (2M XMA purchase limit)`);
    }
  } else {
    throw new Error('Invalid gameType');
  }

  if (cost * credits > MAX_PURCHASE_XMA) {
    throw new Error(`Purchase total exceeds ${MAX_PURCHASE_XMA.toLocaleString()} XMA limit`);
  }
