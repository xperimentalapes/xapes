...
async function restoreFailedPayout({ supabase, payoutId, playersTable, rewardsField }) {
  const { data: payout } = await supabase
    .from('casino_pending_payouts')
    .select('*')
    .eq('id', payoutId)
    .single();
  if (!payout || payout.status !== 'signed') return;
  ...
  await supabase
    ...
    .eq('wallet_address', payout.wallet_address);
  await supabase.from('casino_pending_payouts').update({ status: 'restored' }).eq('id', payoutId);
}
