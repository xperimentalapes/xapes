 ...
 *      SUPABASE_*, MUTANT_APES_COLLECTION_MINT, HELIUS_API_KEY, XMA token mint envs,
...
async function handleMyCasino(interaction, supabase, targetId) {
  ...
  try {
    ...
    if (!cErr && count != null) chestPurchases = count;
  } catch (_) {}

  const totalGames = slotsSpins + rouletteSpins + chestPurchases;
    ...
    embeds: [
      {
        ...
        fields: [
          ...
          { name: 'Chests (purchases)', value: String(chestPurchases), inline: true },
        ],
        footer: {
          text: 'Chests count verified purchases; individual opens are not logged separately.',
        },
      },
    ],
  });
    ...
    embeds: [
      {
        ...
        fields: [
          ...
          {
            name: '/my_casino',
            value: 'Slots spins, roulette spins, chest purchases.',
            inline: false,
          },
          { name: '/nft', value: 'Enter an edition # to show metadata & owner.', inline: false },
          {
            name: 'Admins',
...
async function handleNftModal(interaction, supabase) {
  ...
  const user = interaction.member?.user || interaction.user;
  if (!mint) {
    return { type: 4, data: { embeds: [{ description: 'Collection mint not configured.', color: 0xff0000 }], flags: 64 } };
  }
  ...
  if (!Number.isFinite(edition) || edition < 1) {
    return {
      type: 4,
      data: { embeds: [{ description: 'Enter a valid edition number (1 or higher).', color: 0xff0000 }], flags: 64 },
    };
  }

  const from = edition - 1;
  ...
  if (error) {
    return {
      type: 4,
      data: { embeds: [{ description: `Database error: ${error.message}`, color: 0xff0000 }], flags: 64 },
    };
  }
  ...
  if (!nft) {
    return {
      type: 4,
      data: {
        embeds: [{ description: `No NFT at edition **${edition}**. Run \`npm run populate-nfts\` or wait for sync.`, color: 0xff0000 }],
        flags: 64,
      },
    };
  }
  ...
  const embed = {
    color: 0x2dd4bf,
    title: nft.name || nft.mint_address,
    ...
    footer: { text: `Edition ${edition} · ${nft.mint_address.slice(0, 8)}…` },
  };

  if (traitText.length > 1024) {
    embed.description = traitText.slice(0, 4096);
    embed.fields = [{ name: 'Owner', value: ownerLine.slice(0, 1024), inline: false }];
  }
...
async function processInteraction(interaction, res) {
  ...
  if (interaction.type === 5) {
    ...
    if (!supabase) {
      res.status(200).json({
        type: 4,
        data: { embeds: [{ description: 'Database not configured.', color: 0xff0000 }], flags: 64 },
      });
      return;
    }
  ...
  if (interaction.type !== 2) {
    res.status(200).json({ type: 4, data: { content: 'Unsupported interaction.', flags: 64 } });
    return;
  }
