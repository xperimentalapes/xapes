/**
 * GET /api/discord-rewards/status — session Discord; unclaimed XMA; today (ET) engagement counts + derived daily XMA.
...
const { CASINO_TREASURY_WALLET } = require('../casino/constants');
...
async function publicRewardsMetaPayload() {
  ...
  if (supabase) {
    try {
      ...
      if (!error && data && data.created_at) {
        payload.lastEngagementEventAt = data.created_at;
        const ageMs = Date.now() - new Date(data.created_at).getTime();
        payload.engagementTrackingActive = Number.isFinite(ageMs) && ageMs < 48 * 60 * 60 * 1000;
      }
    } catch (e) {
      console.warn('discord-rewards/meta engagement health', e.message);
    }
  }
...
/** XMA from engagement counts × accrual rates, capped at daily max (UI daily total). */
function engagementClaimEstimateXmaString(messagesCount, reactionsCount, voiceMinutesCount) {
  const r = accrualRates();
  const cap = dailyAccrualCapXma();
  const msgs = Math.max(0, Number(messagesCount) || 0);
  const reacts = Math.max(0, Number(reactionsCount) || 0);
  const vm = Math.max(0, Number(voiceMinutesCount) || 0);
  const raw = r.message * msgs + r.reaction * reacts + r.voiceMinute * vm;
  const capped = Math.min(raw, cap);
  if (!Number.isFinite(capped) || capped <= 0) return '0';
  let s = capped.toFixed(6).replace(/\.?0+$/, '');
  return s || '0';
}
...
async function handlePublicMeta(req, res) {
  ...
  try {
    return res.json(await publicRewardsMetaPayload());
  } catch (e) {
...
function loadTreasuryKeypair() {
  const raw =
    process.env.XMA_REWARDS_TREASURY_KEY ||
    process.env.DISCORD_XMA_REWARDS_WALLET_KEY ||
    process.env.TREASURY_PRIVATE_KEY;
  if (!raw || !String(raw).trim()) return { error: 'XMA_REWARDS_TREASURY_KEY or TREASURY_PRIVATE_KEY not set' };
  const expectedPub = treasuryWalletAddress();
  let keypair;
  try {
...
async function handleStatus(req, res) {
  ...
  const tz = dailyAccrualTimezone();
  const dayStartIso = startOfCurrentDayIsoInTimeZone(tz);
  const dayEndIso = nextMidnightIsoInTimeZone(tz);

  /** Engagement counts for current calendar day in `tz` (same window as accrual date_et). */
  let messagesToday = 0;
  ...
  try {
    ...
    if (msgRes.error) {
      console.error('[discord-rewards/status] messagesToday', msgRes.error.message);
    } else {
    ...
    const reactRes = await supabase
      ...
      .lt('created_at', dayEndIso);
    if (reactRes.error) {
      console.error('[discord-rewards/status] reactionsToday', reactRes.error.message);
    } else {
      reactionsToday = reactRes.count ?? 0;
    }
    ...
    const engagementDateEt = calendarDateInTimeZone(tz);
    let pendingTodayStr = '0';
    const pendingRes = await supabase
      .from('discord_xma_daily_pending')
      .select('pending_xma')
      .eq('discord_user_id', uid)
      .eq('guild_id', gid)
      .eq('accrual_date', engagementDateEt)
      .maybeSingle();
    if (pendingRes.error) {
      console.warn('[discord-rewards/status] discord_xma_daily_pending', pendingRes.error.message);
    } else if (pendingRes.data && pendingRes.data.pending_xma != null) {
      pendingTodayStr = String(pendingRes.data.pending_xma);
    }
    ...
    const voiceMinutesToday = Math.round(voiceMinutes * 100) / 100;
    const todaysClaimXma = engagementClaimEstimateXmaString(
      messagesToday,
      reactionsToday,
      voiceMinutesToday
    );
    ...
    return res.json({
      ...(await publicRewardsMetaPayload()),
      unclaimedXma: unclaimedStr,
      pendingTodayXma: pendingTodayStr,
      todaysClaimXma,
      engagementDateEt,
      ...
      reactions24h: reactionsToday,
      voiceMinutes24h: voiceMinutesToday,
      linkedWallets,
      rewardsTreasuryConfigured: !!(
        process.env.XMA_REWARDS_TREASURY_KEY ||
        process.env.DISCORD_XMA_REWARDS_WALLET_KEY ||
        process.env.TREASURY_PRIVATE_KEY
      ),
      rewardsTreasuryWallet: treasuryWalletAddress(),
    });
  } catch (e) {
...
async function handleClaim(req, res) {
  ...
  async function debitUnclaimedForClaim() {
    const { data: reserveData, error: reserveErr } = await supabase.rpc('reserve_discord_xma_claim', {
      p_discord_user_id: uid,
      p_guild_id: gid,
      p_threshold: threshold,
    });
    if (!reserveErr && reserveData && reserveData.ok === true) {
      return { ok: true, unclaimedStr: String(reserveData.claimed_xma), usedRpc: true };
    }
    if (
      reserveErr &&
      !String(reserveErr.message || '').includes('Could not find the function')
    ) {
    ...
    return { ok: true, unclaimedStr, usedRpc: false };
  }

  async function restoreUnclaimedAfterFailedTx(amount, usedRpc) {
    const n = Number(amount);
    if (!Number.isFinite(n) || n <= 0) return;
    if (usedRpc) {
      await supabase.rpc('restore_discord_xma_claim', {
        p_discord_user_id: uid,
        p_guild_id: gid,
        p_amount: n,
      });
    ...
    const { data: rewardRow } = await supabase
      ...
      .eq('guild_id', gid)
      .maybeSingle();
    const current = rewardRow && rewardRow.unclaimed_xma != null ? Number(rewardRow.unclaimed_xma) : 0;
    const restored = (Number.isFinite(current) ? current : 0) + n;
    await supabase
      .from('discord_xma_rewards')
      .upsert({
        discord_user_id: uid,
        guild_id: gid,
        unclaimed_xma: restored,
  ...
  const debit = await debitUnclaimedForClaim();
  if (!debit.ok) {
    return res.status(debit.status).json(debit.body);
  }

  const unclaimedStr = debit.unclaimedStr;
  const usedClaimRpc = debit.usedRpc;
  const unclaimedNum = Number(unclaimedStr);
  if (!Number.isFinite(unclaimedNum) || unclaimedNum < threshold) {
    await restoreUnclaimedAfterFailedTx(unclaimedNum, usedClaimRpc);
