...
/** Estimated XMA from last-24h engagement counts × accrual rates, capped at daily max (UI “today’s claim”). */
function engagementClaimEstimateXmaString(messages24h, reactions24h, voiceMinutes24h) {
  const r = accrualRates();
  const cap = dailyAccrualCapXma();
  const msgs = Math.max(0, Number(messages24h) || 0);
  const reacts = Math.max(0, Number(reactions24h) || 0);
  const vm = Math.max(0, Number(voiceMinutes24h) || 0);
...
async function handleStatus(req, res) {
  ...
  try {
    ...
    const voiceMinutes24h = Math.round(voiceMinutes * 100) / 100;
    const estimateClaim = engagementClaimEstimateXmaString(
      messages24h,
      reactions24h,
      voiceMinutes24h
    );

    const todayNy = calendarDateInTimeZone(dailyAccrualTimezone());
    let todaysClaimXma = estimateClaim;
    let dailyPendingXma = '0';
    ...
    const pendRes = await supabase
      ...
      .eq('discord_user_id', uid)
      .eq('guild_id', gid)
      .eq('accrual_date', todayNy)
      .maybeSingle();
    if (!pendRes.error && pendRes.data && pendRes.data.pending_xma != null) {
      dailyPendingXma = String(pendRes.data.pending_xma);
      todaysClaimXma = dailyPendingXma;
    }
