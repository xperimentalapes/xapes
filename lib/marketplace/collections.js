/**
 * Collection cards: marketplace stats + metadata (ME API + Helius/Supabase fallbacks).
 * Primary trade link: Orbis (orbisonsol.io).
 */
const axios = require('axios');
const { createClient } = require('@supabase/supabase-js');
const { fetchMeStats, fetchMeCollectionMeta } = require('./magic-eden');
const { getCollectionFloorPriceSol } = require('./floor-price');
...
async function getCollectionsMarketData() {
  ...
  for (const col of cols) {
    const out = {
      ...
      marketplaceUrl: 'https://www.orbisonsol.io/marketplace/' + String(col.slug || '').replace(/_/g, '-'),
    };

    const parallel = [
      fetchMeStats(col.slug, { timeoutMs: 8000 }),
    ...
    if (out.floorPriceSol == null) {
      const fallbackFloor = await getCollectionFloorPriceSol(col.slug, { timeoutMs: 1500 });
      if (fallbackFloor != null) {
        out.floorPriceSol = String(fallbackFloor);
      }
    }
