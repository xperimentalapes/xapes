...
const STATS_STALE_MAX_MS = 24 * 60 * 60 * 1000;
const META_BACKOFF_MS = 5 * 60 * 1000;

/** @type {Map<string, { fetchedAt: number, expires: number, value: object }>} */
const cache = new Map();
/** @type {Map<string, number>} slug -> skip metadata until timestamp */
const metaBackoffUntil = new Map();

function cacheKey(kind, slug) {
  return kind + ':' + slug;
}

function readCache(key, allowStale, staleMaxMs) {
  const hit = cache.get(key);
  if (!hit) return null;
  if (hit.expires > Date.now()) return hit.value;
  if (allowStale && staleMaxMs && Date.now() - hit.fetchedAt < staleMaxMs) return hit.value;
  return null;
}

function writeCache(key, value, ttlMs) {
  const now = Date.now();
  cache.set(key, { fetchedAt: now, expires: now + (ttlMs || CACHE_TTL_MS), value });
}
...
async function meGet(path, timeoutMs) {
  ...
  if (res.status === 429) {
    metaBackoffUntil.set(path.split('/')[2] || 'default', Date.now() + META_BACKOFF_MS);
...
async function fetchMeStats(slug, opts) {
  ...
  try {
    ...
    writeCache(key, out, STATS_CACHE_TTL_MS);
    return out;
  } catch (e) {
    const stale = readCache(key, true, STATS_STALE_MAX_MS);
...
async function fetchMeCollectionMeta(slug, opts) {
  const backoff = metaBackoffUntil.get(slug);
  ...
  try {
    ...
    writeCache(key, out, CACHE_TTL_MS);
    return out;
  } catch (e) {
    if (e.code === 'ME_RATE_LIMIT' || e.code === 'ME_HTTP') {
      metaBackoffUntil.set(slug, Date.now() + META_BACKOFF_MS);
    }
    throw e;
  }
}
