...
async function reconcileDiscordUser(supabase, discordUserId, collectionMint, botToken, guildId, activeRoles, managedIds) {
  ...
  const { data: nftsRows, error: ne } = await supabase
    .from('nfts')
    .select('mint_address, metadata_json, is_crown, is_cowboy, is_business_dao, owner_wallet')
...
async function runFullSync(options = {}) {
  ...
  if (!supabase) throw new Error('Supabase not configured (SUPABASE_URL + SUPABASE_SERVICE_KEY)');
  if (!collectionMint) {
    throw new Error('Collection mint not configured (set MUTANT_APES_COLLECTION_MINT on Vercel)');
  }
  if (mode !== 'roles' && !process.env.HELIUS_API_KEY) {
    throw new Error('HELIUS_API_KEY not set on Vercel — required for NFT sync');
  }

  let nftStats = null;
  let roleStats = null;

  if (mode === 'full' || mode === 'nfts') {
    nftStats = await syncNftsTable(supabase, collectionMint, collectionName);
  }
  if (mode === 'full' || mode === 'roles') {
    roleStats = await reconcileAllDiscordRoles();
  }

  return { mode, nftStats, roleStats };
}
