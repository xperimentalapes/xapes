...
function roleApplies(role, ctx) {
  ...
  if (type === 'nft_column_true') {
    ...
    if (col !== 'is_crown' && col !== 'is_cowboy' && col !== 'is_business_dao') return false;
...
function mergeLiveItemsWithDb(collectionItems, dbRows) {
  ...
  for (const item of collectionItems || []) {
    ...
    // Prefer live Helius attributes when present (DB metadata can be stale).
    const hasLiveAttrs = Array.isArray(meta.attributes) && meta.attributes.length > 0;
    out.push({
      mint_address: mint,
      metadata_json: hasLiveAttrs ? meta : db?.metadata_json || meta,
      is_crown: hasLiveAttrs ? inferred.is_crown : db?.is_crown != null ? !!db.is_crown : inferred.is_crown,
      is_cowboy: hasLiveAttrs ? inferred.is_cowboy : db?.is_cowboy != null ? !!db.is_cowboy : inferred.is_cowboy,
      is_business_dao: hasLiveAttrs
        ? inferred.is_business_dao
        : db?.is_business_dao != null
          ? !!db.is_business_dao
          : inferred.is_business_dao,
    });
  }
...
function ownedNftsFromDbRows(nftsRows) {
  return (nftsRows || []).map((row) => ({
    ...
    is_business_dao: !!row.is_business_dao,
  }));
}
