...
async function getProfileCasinoStats(walletAddress) {
  ...
  const [slotsRes, rouletteRes, coinflipRes, coinflipRoundsRes] = await Promise.all([
    ...
    supabase
      ...
      .eq('wallet_address', wallet)
      .maybeSingle(),
    supabase.from('coinflip_rounds').select('won_amount').eq('wallet_address', wallet),
  ]);

  const slots = slotsRes.data;
  const roulette = rouletteRes.data;
  const coinflip = coinflipRes.data;
  const coinflipRounds = coinflipRoundsRes.data || [];
  const coinflipFlips = coinflipRounds.length;
  const coinflipLifetimeWonRaw = coinflipRounds.reduce(function (sum, row) {
    return sum + Number(row.won_amount || 0);
  }, 0);
  ...
  const games = [
    ...
    },
    {
      ...
      totalWon: fromTokenUnits(coinflipLifetimeWonRaw),
      totalWagered: coinflip ? fromTokenUnits(coinflip.total_wagered) : 0,
      unclaimedRewards: coinflip ? fromTokenUnits(coinflip.total_won) : 0,
