...
const { assertWalletLinkable } = require('./flagged-wallets');
...
module.exports = async function holderVerifyRoles(req, res) {
  ...
  try {
    ...
    assertWalletLinkable(walletAddress);
  } catch (e) {
    if (e.code === 'WALLET_FLAGGED') {
      return res.status(403).json({ error: e.message, rolesSynced: false });
    }
  ...
  let linkedWallets = filterLinkableWallets((linkRows || []).map((r) => r.wallet_address));
  if (!linkedWallets.length) linkedWallets = isStolenWallet(walletAddress) ? [] : [walletAddress];
